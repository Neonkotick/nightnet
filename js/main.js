...
import { initImmersion } from './ui/immersion.js';
import { initNetrunUI, renderNetrun, renderGM } from './ui/netrunUI.js';
...
// ─── Boot ──────────────────────────────────────────────────
function boot() {
  initImmersion(getState().settings?.immersionLevel);
  initNetrunUI();
  renderCategoryChips();
  renderCatalog();
  updateWalletUI();
  ...
  log('NIGHTNET READY // PHASE 3');
  console.log('%cNIGHTNET // ONLINE // PHASE 3', 'color:#00f0ff;font-family:monospace;font-size:14px');
  console.log('NETRUN + GM PANEL active. window.__NIGHTNET__');
}
